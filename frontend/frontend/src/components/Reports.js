...
const Reports = () => {
  ...
  const generateReport = async () => {
    ...
    try {
      ...
      const response = await axios.get('http://localhost:5000/api/reports/borrowing', {
  ...
  const exportToCSV = () => {
    ...
    const headers = ['Borrow ID', 'Student Name', 'Class', 'Book Title', 'Author', 'Borrow Date', 'Return Date', 'Status', 'Days Borrowed'];
    ...
    <div className="reports">
      ...
      {reportData.length > 0 && (
        <div className="report-results">
          <h2>Report Results ({reportData.length} records)</h2>
