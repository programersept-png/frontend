...
const Dashboard = () => {
    ...
    const fetchStats = async () => {
        ...
        try {
            const [booksRes, studentsRes, borrowRes, allBorrowsRes] = await Promise.all([
                ...
                axios.get('http://localhost:5000/api/borrow/current', {
                    headers: { Authorization: `Bearer ${token}` }
                }),
                axios.get('http://localhost:5000/api/borrow', {
                    headers: { Authorization: `Bearer ${token}` }
                })
            ]);
            ...
            const popularBooks = Object.entries(bookBorrowCount)
                .sort((a, b) => b[1] - a[1])
                .slice(0, 5)
                .map(([title, count]) => ({ title: title.length > 25 ? title.substring(0, 25) + '...' : title, count }));
            ...
            // Get recent activities (last 5 borrows)
            const recentActivities = allBorrows.slice(0, 5).map(borrow => ({
                id: borrow.borrow_id,
                student: borrow.student_name,
    ...
    const summaryCards = [
        ...
        },
        {
            ...
            subtitle: `${Math.round((stats.availableBooks / stats.totalBookCopies) * 100)}% available`,
            icon: '📖',
            color: '#2196F3',
            bgColor: '#E3F2FD'
        },
            ...
            <div className="charts-grid">
                ...
                <div className="chart-card">
                    ...
                    <div className="chart-container small-chart">
                        <Doughnut data={availabilityChartData} options={pieChartOptions} />
                    </div>
                </div>
                ...
                {stats.studentsByClass.length > 0 && (
                    <div className="chart-card">
                        ...
                        <div className="chart-container small-chart">
                            <Pie data={classChartData} options={pieChartOptions} />
                        </div>
                    </div>
                )}
                                ...
                                <div key={index} className="activity-item">
                                    ...
                                    <div className="activity-details">
                                        <p className="activity-text">
                                            <strong>{activity.student}</strong> borrowed 
